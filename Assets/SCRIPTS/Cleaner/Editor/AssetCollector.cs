...
namespace Cleaner.Editor
{
    public class AssetCollector
    {
        ...
        public List<string> deleteFileList = new();
        ...
        private List<string> StripTargetPathsAll(bool isUseCodeStrip, string[] pathes)
        {
            ...
            if (!isUseCodeStrip) files = files.Where(item => Path.GetExtension(item) != ".cs");
        ...
        private void UnregistEditorCodes()
        {
            ...
            foreach (var path in editorcodes)
            {
                ...
                if (!referenceCollection.Exists(c => c.fileGuid == guid)) continue;
